...
class PerceptionTaskTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    try {
      detector =
          new HumanDetectorTracker(model_path, config_path, classes_path);
    ...
    if (addPerson) {
      ...
      cv::rectangle(frame,
                    cv::Rect(size.width / 2 - personWidth / 2,
                             size.height / 2 - personHeight / 2, personWidth,
                             personHeight),
                    cv::Scalar(0, 0, 0), -1);
    }
    return frame;
...
TEST_F(PerceptionTaskTest, InvalidPathTest) {
  EXPECT_THROW(
      ...
      cv::Exception)
      << "Should throw OpenCV exception for invalid files";
}
...
TEST_F(PerceptionTaskTest, BasicFrameTest) {
  ...
  EXPECT_NO_THROW({ detector->detectAndTrack(frame); })
...
TEST_F(PerceptionTaskTest, ConsecutiveFrames) {
  ...
  cv::Mat frame =
      createTestFrame(cv::Size(640, 480), true);  // Add a person to track
...
TEST_F(PerceptionTaskTest, EmptyFrameTest) {
  ...
  cv::Mat emptyFrame;
  EXPECT_THROW({ detector->detectAndTrack(emptyFrame); }, cv::Exception)
      << "Should throw exception for empty frame";
}
